...
static glm::mat4 aiMatrixToGlm(const aiMatrix4x4& matrix) {
    glm::mat4 result;
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
...
static glm::mat4 getNodeTransformation(const aiNode* node) {
    return aiMatrixToGlm(node->mTransformation);;
}

static std::unique_ptr<ven::Mesh> processMesh(const aiMesh* mesh, const aiScene* scene, const glm::mat4& transform) {
    auto newMesh = std::make_unique<ven::Mesh>();
    ...
    for (unsigned int i = 0; i < mesh->mNumFaces; i++) {
        ...
        for (unsigned int j = 0; j < face.mNumIndices; j++) {
            newMesh->addIndices(uniqueVertices[ ven::Vertex{ .pos = {
    ...
    if (mesh->mMaterialIndex >= 0) {
        ...
        if (material->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
            ...
            newMesh->setTextureIndex(ven::TextureManager::getTextureIndex("assets/textures/" + std::string(texturePath.C_Str())));
            ...
            newMesh->setTextureIndex(ven::TextureManager::getTextureIndex("assets/textures/default.png"));
        }
    }
    return newMesh;
}

static void processNode(std::vector<std::unique_ptr<ven::Mesh>>& meshes, const aiNode* node, const aiScene* scene, const glm::mat4 &parentTransform) {
    ...
    for (unsigned int i = 0; i < node->mNumChildren; i++) {
        processNode(meshes, node->mChildren[i], scene, globalTransform);
    }
}
...
ven::Model::Model(const std::string& path) {
    ...
    processNode(m_meshes, scene->mRootNode, scene, glm::mat4(1.0F));
}
